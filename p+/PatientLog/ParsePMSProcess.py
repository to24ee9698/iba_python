...
def find_pms_process(tarfileobject, p, lines, room):
    pms = re.compile('^pts-pms-controller-{}-'.format(room))
    if isinstance(tarfileobject,tarfile.TarFile):
        for item in tarfileobject.getmembers():
            if re.search(pms, item.name) is not None:
