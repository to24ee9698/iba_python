...
import ParseScanningController
import ParsePbsRecorder
import ParseSystemLog
        ...
        for tarball in find_tar_files_in_tarball(zipped_tarballs):
            with tarfile.open(fileobj=tarball) as f:
                ...
                for patient in patient_list:
                    ...
                    ParseScanningController.find_scanning_controller(f, patient[0], patient[1])
                    ParsePbsRecorder.find_pbsdr_process(f, patient[0], patient[1])
                    filename = ParseSystemLog.create_search_string(directory, tarball.name)
#                   ParseSystemLog.find_system_log(filename, patient[0], patient[1])
...
main('221076', '/p+/runtimeStore/log/Archive/Output')
