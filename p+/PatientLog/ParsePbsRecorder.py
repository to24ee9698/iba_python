...
def find_pbsdr_process(tarfileobject, p, lines):
    pbsdr = re.compile('^pts-data-recorder-pbs--')
    if isinstance(tarfileobject,tarfile.TarFile):
        for item in tarfileobject.getmembers():
            if re.search(pbsdr, item.name) is not None:
