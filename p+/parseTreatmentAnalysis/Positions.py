...
class Positions(object):
    ...
    def __repr__(self):
        return 'Gantry: {} \tSnout: {}\nPPS\n X {}\tY: {}\tZ {}\nRotation: {}\tPitch: {}\tRoll: {}'.format(
            self.gantry_angle, self.snout, self.pps_x, self.pps_y, self.pps_z,self.pps_rotation, self.pps_pitch,
            self.pps_rotation)
...
def parse_line(line):
    ...
    match = re.search('Gantry:\d*.\d*', line)
    if match is not None:
        first, _, pos.gantry_angle = match.group().partition(':')
    match = re.search('Snout:\d*.\d*', line)
    if match is not None:
